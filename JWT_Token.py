...
SECRET_KEY = "your-secret-key"  # 나중에 gitignore, 환경변수 파일로 바꿔야 함
ALGORITHM = "HS256"
...
def create_refresh_token(data: dict, expires_delta: timedelta | None = None):
    to_encode = data.copy()
    if expires_delta:
        expire = datetime.now(timezone.utc) + expires_delta  # UTC 시간대 인식
    else:
        expire = datetime.now(timezone.utc) + timedelta(days=REFRESH_TOKEN_EXPIRE_DAYS)  # 기본 만료 시간 7일
...
def create_access_token(data: dict, expires_delta: timedelta | None = None):
    ...
    if expires_delta:
        ...
        expire = datetime.now(timezone.utc) + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)  # 기본 만료 시간 15분
    to_encode.update({"exp": expire})  # 만료 시간 포함
    encoded_jwt = jwt.encode(to_encode, SECRET_KEY, algorithm=ALGORITHM)
    return encoded_jwt

# Refresh_token으로 Access_token발급
...
async def refresh_access_token(refresh_token: str):
    try:
        ...
        # 새로운 Access Token 발급
        user_data = {"sub": payload.get("sub"), "member_id": member_id}
        access_token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
        new_access_token = create_access_token(data=user_data, expires_delta=access_token_expires)
...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    return member_id  # 토큰의 member_id 반환

# 준서
# 로그인 엔드포인트 - 여기서는 refresh_access_token이 아니라 create_access_token, create_refresh_token을 사용해야 함
...
async def login(request: LoginRequest, db: Session = Depends(get_db)):
    try:
        ...
        user_data = {"sub": user.login_id, "member_id": user.id}
        access_token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
        ...
        # Access Token과 Refresh Token을 응답으로 반환
        return {
        ...
        raise HTTPException(status_code=500, detail=str(e))  # 예외 발생 시 에러 메시지를 반환
...
async def get_member_by_id(
    ...
    current_user_id: int = Depends(get_current_user),  # 현재 로그인된 사용자 정보
):
    try:
        # 요청한 member_id와 현재 로그인한 사용자의 ID를 비교 (필요 시)
